...
func (g *generateHTTPTransportBase) Generate() (err error) {
	...
	g.srcFile.PackageComment("Package http THIS FILE IS AUTO GENERATED BY GK-CLI DO NOT EDIT!!")
	endpointImport, err := utils.GetEndpointImportPath(g.name)
	if err != nil {
		return err
	}
...
func (g *generateGRPCTransportBase) Generate() (err error) {
	...
	g.srcFile.PackageComment("Package grpc THIS FILE IS AUTO GENERATED BY GK-CLI DO NOT EDIT!!")
	endpointImport, err := utils.GetEndpointImportPath(g.name)
	if err != nil {
		return err
	}
...
func (g *generateGRPCTransport) Generate() (err error) {
	...
	for _, m := range g.serviceInterface.Methods {
		...
		if !decoderFound {
			g.code.appendMultilineComment([]string{
				fmt.Sprintf("decode%sRequest is a transport/grpc.DecodeRequestFunc that converts a", m.Name),
